...
class _ARShow3DModelAtGeolocationWidgetState extends State<ARShow3DModelAtGeolocationWidget> with WidgetsBindingObserver {
  //AR setup
  ...
  Future<void> checkGPS() async {
    ...
    if (servicestatus){
      ...
      if (haspermission){
        ...
        return Future.error('GPS servicde is not enabled, turn on GPS location services');
      }
    }
    setState(() {});
  }
  ...
  Future<void> getLocation() async{
    ...
    positionStream = Geolocator.getPositionStream(locationSettings: locationSettings).listen((Position position){
      // debugPrint(" getting the user position");
      // debugPrint(" The position: $position");
      //UPDATING THE AR.CONTEXT
      architectWidget.setLocation(position.latitude, position.longitude, position.altitude, position.accuracy);
    });
  }
